...
class GNN1(nn.Module):
    ...
    def forward(self, datas, device=None):
        if device is None:
        ...
        drug_rel_weigh = drug_rel.matmul(self.W1.to(device)) + self.b1.to(device)
        drug_rel_weigh = self.relu(drug_rel_weigh)
        drug_rel_weigh = drug_rel_weigh.matmul(self.W2.to(device)) + self.b2.to(device)
...
class GNN2(nn.Module):
    ...
    def forward(self, arguments, device=None):
        if device is None:
            device = self.drug_name_tensor.device
        kg, dict1, args = self.kg, self.dict1, self.args
