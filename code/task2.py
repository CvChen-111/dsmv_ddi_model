...
def train(train_x, train_y, test_x, test_y, net, test_adj):
    ...
    train_x_total = torch.tensor(np.concatenate([train_x1, train_x], axis=0), dtype=torch.long)
    ...
    test_list = []
    max_test_output = np.zeros((0, 65), dtype=float)
    for epoch in range(args.epoches):
        ...
        for x, y in train_iter:
            ...
            train_acc = accuracy_score(preds, y.cpu().numpy())
        ...
        with torch.no_grad():
            ...
            test_score = f1_score(test_preds, test_label.cpu().numpy(), average='macro')
            test_acc = accuracy_score(test_preds, test_label.cpu().numpy())
        ...
        print('epoch [%d] train_loss: %.6f testing_loss: %.6f ' % (
            epoch + 1, train_l / len(train_y_tensor), test_loss / len(test_y)))
    return test_loss / len(test_y), max(test_list), train_l / len(train_y_tensor), sum(train_a) / len(train_a), test_list, max_test_output
